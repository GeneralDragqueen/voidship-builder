...
If saved data needs repair, the app keeps an original recovery copy and offers **Download original data**. Harmless legacy defaults do not trigger recovery, and an identical original is reused rather than archived again. **Saved recovery copies** lets you download individual originals and explicitly delete copies you no longer need; unique copies are never removed automatically. A copy needed to protect an unfinished recovery cannot be deleted.

If a recovery copy cannot be stored, recovery runs in the current tab without overwriting the original storage. Download the original data and a backup before closing. You can remove an unneeded older copy to free space and use **Retry saving** to save the recovered ships and current edits.
...
`npm test` copies only the public files into `_site/` and tests that artifact under `/voidship-builder/`, matching the Pages URL prefix. The offline smoke test opens the same staged HTML directly. To run the same regressions against an older extracted site, use `SITE_ROOT=/absolute/path/to/old-site npx playwright test`; the test server never modifies the selected site. Test reports, browser traces, and screenshots are kept outside the publishable artifact.

`ledger/` holds a companion reference sheet (LaTeX source and PDF) of the same component tables.

## Publication

The **Test and publish** workflow tests pull requests and changes to `main`. Only a passing `main` run can package and deploy the exact site files tested by that run. A manual workflow run also requires `main` to publish. New pull-request runs cancel superseded checks, while an active `main` run can finish publishing before a newer run starts. Reports and failure screenshots are separate artifacts and are never published with the site.
